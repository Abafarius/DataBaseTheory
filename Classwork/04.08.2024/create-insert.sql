...
CREATE VIEW TASK AS 
SELECT A.FIO AS 'AUTHOR', B.PUBLICATIONYEAR AS 'YEAR', B.BOOKNAME AS 'BOOK'
...
CREATE VIEW TASK0 AS 
SELECT R.FIO AS 'CLIENT', B.BOOKNAME AS 'BOOK'
FROM BOOK AS B JOIN ORDERS AS O
ON O.BOOKID = B.id JOIN READER AS R
ON R.id = O.READERID
WHERE YEAR(O.STARTDATE) = 2023
...
SELECT A.FIO AS 'AUTHOR', B.BOOKNAME AS 'BOOK', R.FIO AS 'READER'
FROM AUTHOR AS A JOIN BOOK AS B
ON A.ID = B.AUTHORID JOIN ORDERS AS O
ON O.bookId = B.ID JOIN READER AS R
ON R.ID = O.readerId
...
SELECT B.BOOKNAME AS 'BOOKNAME', COUNT(*) AS 'COUNT'
FROM BOOK AS B JOIN ORDERS AS O
ON O.BOOKID = B.ID JOIN READER AS R
ON R.ID = O.READERID
GROUP BY B.bookName
...
--***********************************TRIGGER - IT'S AN ACTION, WHICH OCCURES WHEN ************************************************
/*
“–»√√≈– - ¬≈ÿ¿≈“—ﬂ Õ¿ ŒƒÕ” “¿¡À»÷”, Õ¿«€¬¿≈“—ﬂ  ¿  ƒ≈…—“¬»≈,    Œ“Œ–ŒÃ” œ–»¬ﬂ«¿Õ ƒ¬¿ “»œ “–»√√≈–¿:
AFTER - —–¿Œ“¿“≈“ œŒ—À≈ Œœ≈–¿÷»»
INSETAD OF - —–¿¡Œ“¿≈“ ¬Ã≈—“Œ Œœ≈–¿÷»»
œ–» –¿¡Œ“≈ — “–»√√≈–¿Ã» —Œ«ƒ¿ﬁ“—ﬂ ƒ¬≈ ¬–≈ÃÕÕ€≈ “¿¡À»÷€, ≈—À» 
INSERT/UPDATE - INSERTED
DELETE - DELETED
›“» “¿¡À»◊ » —Œƒ≈–∆¿“ ŒƒÕ” —“–Œ ”, —  Œ“Œ–Œ… –¿¡Œ“¿≈Ã
*/

CREATE TRIGGER READER_INSERT 
ON READER
INSTEAD OF INSERT 
AS 
	SELECT *
...
--◊“Œ¡€ œ–Œ¬≈–»“‹ –¿¡Œ“” “–»√√≈–¿ —Œ¬≈–ÿ¿≈Ã Œœ≈–¿÷»ﬁ
INSERT INTO READER
VALUES ('FIRST TEST', '2020-05-05', '789789', 'ABAY')

ALTER TRIGGER READER_INSERT 
ON READER
INSTEAD OF INSERT 
AS 
	DECLARE @ISEXIST INT = (SELECT COUNT(*)
							FROM READER
							WHERE FIO = (SELECT FIO
										FROM INSERTED))
	IF (@ISEXIST = 0)
	BEGIN
		INSERT INTO READER (FIO, BDATE, PHONE, ADDRESS)
		SELECT FIO, BDATE, PHONE, ADDRESS
		FROM INSERTED
	END 
...
UPDATE BOOK
SET ISRETURN = 1
WHERE AUTHORID = (SELECT ID FROM DELETED)
